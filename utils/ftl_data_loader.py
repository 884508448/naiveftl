...
class FTLDataLoader:
    ...
    def __init__(self, file_name):
        self.data_inst: pd.DataFrame = self.__load_data(file_name)
        self.id_index_map: dict = dict(zip(self.data_inst['id'], range(len(self.data_inst['id']))))
        if 'y' in self.data_inst.columns:
            self.labels: np.array = np.array(self.data_inst['y'])
        self.data_matrix: np.matrix = np.matrix(self.data_inst[self.data_inst.columns.difference(['id', 'y'])])
    ...
    def __load_data(self, file_name):
        ...
        try:
            ...
            LOGGER.error(f"file not found, please check the path: '{file_name}', only supports .csv file now")
            raise e

    def __label_check(self, data_frame):
        label_set = set(data_frame['y'])
        assert len(label_set) == 2, f"only supprots 2 classes of labels, but {label_set} is offered"
        if label_set != {-1, +1}:
            label_list = list(label_set)
            LOGGER.info(f"mapping label value {label_list[0]} to -1, {label_list[1]} to +1")
